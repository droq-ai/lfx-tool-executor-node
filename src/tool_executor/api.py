...
async def get_nats_client():
    ...
    if _nats_client is None:
        ...
        from tool_executor.nats import NATSClient
    ...
    if isinstance(result, BaseTool):
        ...
        try:
            ...
            if hasattr(result, "metadata") and result.metadata:
                print(f"[SERIALIZE_RESULT] 🔧 Tool '{tool_name}' has metadata: {list(result.metadata.keys())}", flush=True)
                if "_component_state" in result.metadata:
                    ...
                    if isinstance(comp_state, dict) and "parameters" in comp_state:
                        ...
                        print(f"[SERIALIZE_RESULT] 🎯 Tool '{tool_name}' _component_state['parameters']['api_key'] = {repr(api_key_val)}", flush=True)
            ...
            print(f"[SERIALIZE_RESULT] ✅ Serialized Tool '{tool_name}': metadata keys = {list(serialized.get('metadata', {}).keys())}", flush=True)
            if "_component_state" in serialized.get("metadata", {}):
                print(f"[SERIALIZE_RESULT] ✅ Tool '{tool_name}' _component_state preserved in serialized result!", flush=True)
            return serialized
        except Exception as exc:
            print(f"[SERIALIZE_RESULT] ❌ Failed to serialize tool '{tool_name}': {exc}", flush=True)
            import traceback
...
async def execute_component(request: ExecutionRequest) -> ExecutionResponse:
    ...
    try:
        ...
        if request.component_state.component_class == "AgentQL" and "api_key" in component_params:
            ...
            print(f"[EXECUTOR] 🎯 AgentQL API KEY received in component_state.parameters: {repr(api_key_val)}", flush=True)
        ...
        if request.component_state.component_class == "AgentQL":
            if hasattr(component, "api_key"):
                ...
                print(f"[EXECUTOR] 🎯 AgentQL component.api_key attribute AFTER instantiation: {repr(api_key_attr)}", flush=True)
                logger.info(f"[EXECUTOR] 🎯 AgentQL component.api_key attribute: {repr(api_key_attr)}")
            else:
                print(f"[EXECUTOR] ⚠️ AgentQL component has NO api_key attribute after instantiation!", flush=True)
        ...
        if request.component_state.stream_topic:
            ...
            component._stream_topic_from_backend = request.component_state.stream_topic  # noqa: SLF001
        ...
        if request.method_name == "to_toolkit" and request.component_state.component_class == "AgentQL":
            print(f"[EXECUTOR] 🎯 Executing to_toolkit for AgentQL component", flush=True)
            api_key_in_params = request.component_state.parameters.get("api_key")
            print(f"[EXECUTOR] 🎯 AgentQL api_key in component_state.parameters BEFORE to_toolkit: {repr(api_key_in_params)}", flush=True)
            # Also check if component instance has api_key
            if hasattr(component, "api_key"):
                print(f"[EXECUTOR] 🎯 AgentQL component.api_key attribute: {repr(getattr(component, 'api_key', None))}", flush=True)
        ...
        if request.method_name == "to_toolkit" and request.component_state.component_class == "AgentQL":
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                if hasattr(first_tool, "metadata"):
                    print(f"[EXECUTOR] 🎯 First tool metadata keys: {list(first_tool.metadata.keys()) if first_tool.metadata else 'NONE'}", flush=True)
                    if first_tool.metadata and "_component_state" in first_tool.metadata:
                        comp_state = first_tool.metadata["_component_state"]
                        if isinstance(comp_state, dict) and "parameters" in comp_state:
                            params = comp_state["parameters"]
                            api_key_val = params.get("api_key") if isinstance(params, dict) else None
                            print(f"[EXECUTOR] 🎯 First tool _component_state['parameters']['api_key']: {repr(api_key_val)}", flush=True)
                    else:
                        print(f"[EXECUTOR] ⚠️ First tool has NO _component_state in metadata!", flush=True)
